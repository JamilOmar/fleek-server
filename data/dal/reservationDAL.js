...
//Method to select the Reservation by Customer Id and State
//*******************************************************************************************
reservationDAL.prototype.getReservationByCustomerIdStatePaged = function(id,state,offset,limit, resultMethod,connection) {
    var getReservationByCustomerIdStatePagedQuery ="SELECT  reservation.`ReservationId` ,reservation.`ProviderScheduleId`,  reservation.`Address` , reservation.`CancelationReason` , reservation.`Date` , reservation.`EndTime` , reservation.`StartTime`, reservation.`Latitude` , reservation.`Longitude`, reservation.`State` , reservation.`CreationDate` , reservation.`ModificationDate` , reservation.`IsActive` ,customer.`UserId` as 'customer_UserId', customer.`Name` as 'customer_Name', customer.`Lastname` as 'customer_Lastname', customer.`Username` as 'customer_Username' , customer.`PictureUrl` as 'customer_PictureUrl',customer.`FacebookId` as 'customer_FacebookId',provider.`UserId` as 'provider_UserId', provider.`Name` as 'provider_Name', provider.`Lastname` as 'provider_Lastname', provider.`Username` as 'provider_Username' , provider.`PictureUrl` as 'provider_PictureUrl',provider.`FacebookId` as 'provider_FacebookId'   FROM `Reservation` reservation INNER JOIN `User` customer ON customer.`UserId` = reservation.`CustomerId` INNER JOIN `User` provider ON provider.`UserId` = reservation.`ProviderId` WHERE  reservation.`IsActive` = 1 AND customer.`IsActive` =1 AND provider.`IsActive` = 1 AND provider.`IsBlocked` = 0 and customer.`IsBlocked` = 0 and reservation.`CustomerId` =? AND reservation.`State` =? Order by Date ASC LIMIT ?,?";
                reservationDAL.prototype.getByArguments(getReservationByCustomerIdStatePagedQuery,[id,state,offset,limit],function (err,result)
                {
                    logger.log("debug","getReservationByCustomerIdStatePaged",id , result);
                    return resultMethod(err,reservationDAL.prototype.self.mapperSqlToModelCollection(result));
                },connection);  
};

//Method to select the Reservation by Provider Id and State
//*******************************************************************************************
reservationDAL.prototype.getReservationByProviderIdStatePaged = function(id,state,offset,limit, resultMethod,connection) {
    var getReservationByProviderIdStatePagedQuery ="SELECT  reservation.`ReservationId` ,reservation.`ProviderScheduleId`,  reservation.`Address` , reservation.`CancelationReason` , reservation.`Date` , reservation.`EndTime` , reservation.`StartTime`, reservation.`Latitude` , reservation.`Longitude`, reservation.`State` , reservation.`CreationDate` , reservation.`ModificationDate` , reservation.`IsActive` ,customer.`UserId` as 'customer_UserId', customer.`Name` as 'customer_Name', customer.`Lastname` as 'customer_Lastname', customer.`Username` as 'customer_Username' , customer.`PictureUrl` as 'customer_PictureUrl',customer.`FacebookId` as 'customer_FacebookId',provider.`UserId` as 'provider_UserId', provider.`Name` as 'provider_Name', provider.`Lastname` as 'provider_Lastname', provider.`Username` as 'provider_Username' , provider.`PictureUrl` as 'provider_PictureUrl',provider.`FacebookId` as 'provider_FacebookId'   FROM `Reservation` reservation INNER JOIN `User` customer ON customer.`UserId` = reservation.`CustomerId` INNER JOIN `User` provider ON provider.`UserId` = reservation.`ProviderId` WHERE  reservation.`IsActive` = 1 AND customer.`IsActive` =1 AND provider.`IsActive` = 1 AND provider.`IsBlocked` = 0 AND customer.`IsBlocked` = 0 AND reservation.`ProviderId` =? AND reservation.`State` =? Order by Date ASC LIMIT ?,?";
...
reservationDAL.prototype.mapperSqlToModel = function(data)
{
    try
    {
        ...
        if(data != null && data.length >0)
        {
           ...
           Reservation.provider.basicInformation(data.provider_UserId ,data.provider_Name, data.provider_Lastname , data.provider_Username , data.provider_PictureUrl,data.provider_FacebookId);
           Reservation.customer = new userModel();
           Reservation.customer.basicInformation(data.customer_UserId ,data.customer_Name, data.customer_Lastname , data.customer_Username , data.customer_PictureUrl,data.customer_FacebookId);
